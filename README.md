...
|:---|:---|:---|:---:|
| DeepSeek | deepseek-chat | Chat | ❌ 需自行配置 |
| 智谱 AI | glm-4-flash, glm-4 | Chat | ❌ 需自行配置|
| 智谱 AI | embedding-2 | Embedding | ❌ 需自行配置 |
| 通义千问 | qwen-turbo, qwen-plus | Chat | ❌ 需自行配置 |
| 豆包 | doubao-pro-32k | Chat | ❌ 需自行配置 |

> 💡 **说明**：需要在管理后台「模型管理」中自行配置 API Key。
