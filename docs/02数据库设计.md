...
| **Workflow** | `ai_workflow_execution` | 工作流执行历史 |
...
### 2.6 Workflow 域 - 工作流执行历史

#### 2.6.1 `ai_workflow_execution` 工作流执行历史表

记录工作流每次执行的详细信息，用于调试与监控。
...
|--------|----------|----------|--------|------|
| id | bigint | 是 | AUTO_INCREMENT | 主键 |
| workflow_id | varchar(32) | 是 | - | 工作流ID |
| session_id | bigint | 否 | NULL | 会话ID |
| user_id | varchar(32) | 否 | NULL | 用户ID |
| status | varchar(20) | 是 | - | 执行状态: success-成功, failed-失败, partial-部分成功 |
| start_time | datetime | 是 | - | 开始时间 |
| end_time | datetime | 否 | NULL | 结束时间 |
| duration_ms | int | 否 | NULL | 执行耗时（毫秒） |
| input_data | json | 否 | NULL | 输入数据（用户输入、上下文等） |
| output_data | json | 否 | NULL | 输出数据（回复内容、情绪等） |
| node_executions | json | 否 | NULL | 各节点执行结果 |
| error_message | text | 否 | NULL | 错误信息 |
| created_at | datetime | 是 | CURRENT_TIMESTAMP | 创建时间 |
...
CREATE TABLE `ai_workflow_execution` (
  ...
  `input_data` json DEFAULT NULL COMMENT '输入数据（用户输入、上下文等）',
  `output_data` json DEFAULT NULL COMMENT '输出数据（回复内容、情绪等）',
...
**场景支撑**：工作流执行监控、问题排查、性能分析

**实现说明**：
- 每次工作流执行都会记录一条历史记录
- `node_executions` 字段存储各节点的执行详情，便于定位问题节点
- `duration_ms` 记录执行耗时，用于性能监控
- 可通过 `session_id` 查询某次对话的工作流执行情况
- 历史数据可定期归档或清理

**`node_executions` 字段结构示例**：
...
[
  ...
  {
    "nodeId": "node_safety_check",
    "nodeType": "safety_check",
    "status": "success",
    "result": "safe",
    "startTime": "2024-01-01T12:00:00",
    "endTime": "2024-01-01T12:00:01",
    "durationMs": 120
  },
  {
    "nodeId": "node_llm_process",
    "nodeType": "llm_process",
    "status": "success",
    "tokenCount": 150,
    "startTime": "2024-01-01T12:00:01",
    "endTime": "2024-01-01T12:00:05",
    "durationMs": 4200
  }
]
```
...
| `sys_user` | `idx_role_status` | 按角色和状态组合筛选用户 |
...
| `ai_workflow_execution` | `idx_workflow_id` | 按工作流查询执行历史 |
| `ai_workflow_execution` | `idx_session_id` | 按会话查询执行历史 |
| `ai_workflow_execution` | `idx_created_at` | 按时间查询执行记录 |
